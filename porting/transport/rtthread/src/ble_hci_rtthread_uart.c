 ...
 * 2025-01-19     Evlers       update hci h4
 */
...
static void rtthread_hci_rx_entry(void *parameter)
{
    ...
    rt_device_set_rx_indicate(hci_dev, rx_indicate);
...
static int rtthread_hci_uart_init(void)
{
    ...
    rt_device_control(hci_dev, RT_DEVICE_CTRL_CONFIG, &config);

#ifdef RT_USING_SERIAL_V2
    rt_device_open(hci_dev, RT_DEVICE_FLAG_RX_NON_BLOCKING | RT_DEVICE_FLAG_TX_BLOCKING);
#else
    rt_device_open(hci_dev, RT_DEVICE_FLAG_INT_RX);
#endif

    rt_thread_t rx_thread = rt_thread_create("hci_uart_rx", rtthread_hci_rx_entry, RT_NULL, 1024, 25, 10);
    if (rx_thread != RT_NULL) {
        rt_thread_startup(rx_thread);
    } else {
        LOG_E("hci rx thread create failed");
        return -1;
    }
